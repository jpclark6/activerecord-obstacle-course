...
describe 'ActiveRecord Obstacle Course' do
  ...
  it "13. groups an order's items by name" do
    ...
    # ------------------ Using ActiveRecord ----------------------
    grouped_items = Order.find(3).items.order("items.name asc")
    # ------------------------------------------------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.joins(:orders, :items).where(order_items: {item_id: item_8.id}).distinct.pluck(:name)
    # ------------------------------------------------------------
  ...
  it '22. calculates the total sales for all but one user' do
    ...
    # ------------------ Using ActiveRecord ---------------------
    total_sales = Order.where("user_id != 2")
                        .sum(:amount)
    # -----------------------------------------------------------
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.where({user: user_2, order_items: {item: item_4}})
